...
{
    int healthLeds[] = {1,2,3,4,5,7,8,9,10,11,13,14,15,16,17,19,20,21,22,23}; // the location of the 20 LEDs used for health
  
    leds[healthLeds[0]] = HEALTHCOLOR*(health > 0);     // last LED doesn't go off till the health is 0
    leds[healthLeds[19]] = HEALTHCOLOR*(health == 20);  // first LED goes off as soon as the health is not max health

    for(int i = 1; i < 19; i++)
    {
        leds[healthLeds[i]] = HEALTHCOLOR*(health > (i*1)); // the other leds go off in increments
    }
}
